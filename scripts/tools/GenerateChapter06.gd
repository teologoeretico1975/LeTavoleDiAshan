# scripts/tools/GenerateChapter06.gd
#
# EditorScript che genera data/levels/chapter_06.json.
# Come eseguire: apri nell'editor Godot, premi Run (▶) o Ctrl+Shift+X.
#
# Range 48-52, stesso ceiling del Capitolo V — stati diversi, puzzle differenti.
# Differenza chiave: par_moves = round(optimal * 1.2) invece di 1.4
# → le 3 stelle richiedono quasi il percorso ottimale, difficoltà percepita massima.
#
# Se MD=52 risulta instabile (come nel Capitolo V), si accettano 19/20 livelli.
...
# Range 48-52 = 5 valori distinti → 4 livelli ciascuno = 20 totali.
const TARGETS: Array[int] = [
	48, 48, 48, 48, 49, 49, 49, 49, 50, 50,
	50, 50, 51, 51, 51, 51, 52, 52, 52, 52,
]

const OUTPUT_PATH := "res://data/levels/chapter_06.json"


func _run() -> void:
	print("=".repeat(40))
	print("Generazione Capitolo VI — 20 livelli 4x4 (MD-increasing)")
	print("par_moves = optimal * 1.2  (3 stelle difficili)")
	print("=".repeat(40))

	var rng := RandomNumberGenerator.new()
	rng.seed = 0x41534136  # "ASA6" in ASCII
	...
	for i: int in TARGETS.size():
		...
		level_num += 1
		# par_moves = 1.2× optimal: soglia 3 stelle molto più esigente del Capitolo V (1.4×).
		var par_moves:  int = roundi(target * 1.2)
		var good_moves: int = target * 2

		levels.append({
			"id":            level_num,
			"title":         "Livello VI-%d" % level_num,
		...
		print("  [%d/%d] optimal=%d  par=%d  (%d ms)" \
			% [level_num, TOTAL_LEVELS, target, par_moves, elapsed])

	print("-".repeat(40))
	if unreachable.is_empty():
		print("MD-increasing raggiungibile su tutto il range 48-52.")
	else:
